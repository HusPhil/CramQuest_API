...
async def crud_create_player(
    session: AsyncSession, user_id: int, player_create: PlayerCreate
) -> PlayerRead:
    ...
    try:
        ...
        raise RuntimeError(
            f"Unexpected SQLAlchemyError while creating Player: {str(e)}"
        )
...
async def crud_read_player_with_user(
...
async def crud_read_all_players_with_users(session: AsyncSession) -> List[PlayerRead]:
    ...
    statement = select(Player).options(joinedload(Player.user))
...
async def crud_read_player_profile(
    session: AsyncSession, player_id: int
) -> ProfileRead:
...
async def _get_player_with_subject_or_error(
    session: AsyncSession, player_id: int
) -> Player:
    statement = (
        select(Player)
...
async def _get_user_and_player_or_error(
