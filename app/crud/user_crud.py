...
async def crud_create_user(session: AsyncSession, user_create: UserCreate) -> UserRead:
    ...
    _check_existing_user_based_on_email_and_username(
        session, user_create.username, user_create.email
    )
...
async def crud_read_user_player(session: AsyncSession, user_id: int) -> PlayerRead:
    ...
    return PlayerRead(
        ...
        boss_availability_counter=user_player.boss_availability_counter,
...
async def crud_update_user(
    ...
    if user_update.password:
        update_data["password"] = Security.hash_string(
    ...
    if not update_data:
        ...
        )  # ✅ No Updates, Return Existing Data
    ...
    try:
        ...
        await session.rollback()
        raise HTTPException(
            status_code=500, detail=f"Unexpected SQLAlchemyError: {str(e)}"
        )
...
async def _check_existing_user_based_on_email_and_username(
